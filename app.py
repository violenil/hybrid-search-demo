...
def weighted_sum(bm25_scores, vector_scores, weights={'bm25': 0.4, 'vector': 0.6}):
    ...
    bm25_scores_dict = {
        result['id']: result['score'] for result in normalized_bm25_scores
    }
    vector_scores_dict = {result['id']: result['score'] for result in vector_scores}
    all_ids = set(
        [result['id'] for result in bm25_scores]
        + [result['id'] for result in vector_scores]
    )
    for id in all_ids:
    ...
    combined_scores = []
    for id in all_ids:
        combined_score = (weights['bm25'] * bm25_scores_dict[id]) + (
            weights['vector'] * vector_scores_dict[id]
        )
        combined_scores.append(
            {'score': combined_score, 'id': id, 'text': documents[id]}
        )
...
def bm25_search(query, limit=20):
    ...
    return [{'text': documents[i], 'score': scores[i], 'id': i} for i in top_n]


def hybrid_search(bm25_results, vector_results):
    # Combine scores using weighted sum
    combined_scores = weighted_sum(
        bm25_results, vector_results
    )  # [{'score': combined_score, 'id': doc_id}]
...
def vector_search(query, k=20):
    ...
    cosine_scores = [
        cosine_similarity(query_embedding, [vector]) for vector in valid_vectors
    ]
    # Return results with valid indices and distances
    results = [
        {'text': documents[idx], 'score': float(score), 'id': idx}
        for score, idx in zip(cosine_scores, valid_indices)
    ]
...
def search():
    ...
    display_results_bm25 = [
        {'text': result['text'], 'score': result['score']} for result in bm25_results
    ]
    display_results_vector = [
        {'text': result['text'], 'score': result['score']} for result in vector_results
    ]
    display_results_hybrid = [
        {'text': result['text'], 'score': result['score']} for result in hybrid_results
    ]

    return jsonify(
        bm25=display_results_bm25,
        vector=display_results_vector,
        hybrid=display_results_hybrid,
    )
